...
#define zip_strcpy(des, src)    (strcpy_s(des, src))
#else
#define zip_strcpy(des, src)    (strcpy(des, src))
...
int zip_entry_close(struct zip_t *zip) {
    ...
    dos_time = (mz_uint16)(((time_info->tm_hour) << 11) + ((time_info->tm_min) << 5) + ((time_info->tm_sec) >> 1));
    dos_date = (mz_uint16)(((time_info->tm_year + 1900 - 1980) << 9) + ((time_info->tm_mon + 1) << 5) + time_info->tm_mday);
...
#if defined _WIN32 || defined __WIN32__ || defined __EMX__ || defined __DJGPP__
        strncpy_s(&path[dirlen], MAX_PATH + 1 - dirlen, info.m_filename, MAX_PATH - dirlen);
#else
...
class Zip::Impl {
  public:
    Impl(const std::string& zip_file_name, uint8_t compression_level/* = 6*/, bool append/* = false*/)
            : zip_file_name_(zip_file_name)
    ...
    bool AddBuffer(const std::string& buffer_name, std::istream& buffer, bool is_end_buffer = true);
    void Close();
    void Reopen() { append_ = true; }
    
    bool UnZip(const std::string& unzip_dir);
    
  private:
    std::string zip_file_name_;
...
bool Zip::Impl::AddFile(const std::string& file_name) {
    return AddFile(file_name, file_name);
}

bool Zip::Impl::AddFile(const std::string& file_name_in_zip, const std::string& file_name) {
    if (!zip_)
        zip_ = zip_open(zip_file_name_.c_str(), compression_level_, append_);
    
    if (!zip_)
        return false;
    
    if (zip_entry_open(zip_, file_name_in_zip.c_str()) != 0)
        return false;
    
    if (zip_entry_fwrite(zip_, file_name.c_str()) != 0) {
        zip_entry_close(zip_);
        return false;
    }
    
    return zip_entry_close(zip_) == 0;
}

bool Zip::Impl::AddBuffer(const std::string& buffer_name, std::istream& buffer, bool is_end_buffer/* = true*/) {
    if (!zip_)
        zip_ = zip_open(zip_file_name_.c_str(), compression_level_, append_);
    ...
    if (zip_entry_open(zip_, buffer_name.c_str()) != 0)
        return false;
    
    char buf[2] = { 0 };
    buffer.seekg(0, std::ios::end);
    auto buffer_size = buffer.tellg();
    buffer.seekg(0);
    std::streamsize read_count = 0;
    while (buffer_size > 0) {
        buffer.read(buf, sizeof buf);
        read_count = buffer.gcount();
        
        if (zip_entry_write(zip_, buf, static_cast<size_t>(read_count)) == -1) {
            zip_entry_close(zip_);
            return false;
        }
...
bool Zip::Impl::UnZip(const std::string& unzip_dir) {
    Close();
    return zip_extract(zip_file_name_.c_str(), unzip_dir.c_str(), nullptr, nullptr) == 0;
}

Zip::Zip(const std::string& zip_file_name, uint8_t compression_level/* = 6*/, bool append/* = false*/) {
    impl_ = std::make_shared<Impl>(zip_file_name, compression_level, append);
}

bool Zip::AddFile(const std::string& file_name) {
    return impl_ ? impl_->AddFile(file_name) : false;
}

bool Zip::AddFile(const std::string& file_name_in_zip, const std::string& file_name) {
    return impl_ ? impl_->AddFile(file_name_in_zip, file_name) : false;
}

bool Zip::AddBuffer(const std::string& buffer_name, std::istream& buffer, bool is_end_buffer/* = true*/) {
    return impl_ ? impl_->AddBuffer(buffer_name, buffer, is_end_buffer) : false;
}
...
bool Zip::UnZip(const std::string& unzip_dir) {
    return impl_ ? impl_->UnZip(unzip_dir) : false;
}
}
